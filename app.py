...
# ---- Helper: Add Buy/Sell Signals ----
def add_signals(df):
    df = df.copy()
    df["signal"] = 0
    df.loc[(df["fast_ma"] > df["slow_ma"]) & (df["rsi"] < 30), "signal"] = 1  # Buy
    df.loc[(df["fast_ma"] < df["slow_ma"]) & (df["rsi"] > 70), "signal"] = -1  # Sell
    return df
        ...
        try:
            ...
            if row.empty:
                ...
                if df_hist.empty:
                    ...
                    df_hist = add_signals(df_hist)

                    # ---- Historical Candlestick Chart with Signals ----
                    ...
                    fig_hist.add_trace(go.Scatter(x=df_hist['date'], y=df_hist['fast_ma'],
                                                  line=dict(color='blue', width=1), name='Fast MA'))
                    fig_hist.add_trace(go.Scatter(x=df_hist['date'], y=df_hist['slow_ma'],
                                                  line=dict(color='orange', width=1), name='Slow MA'))
                    fig_hist.add_trace(go.Scatter(x=df_hist['date'], y=df_hist['bb_high'],
                                                  line=dict(color='green', width=1, dash='dot'), name='BB High'))
                    fig_hist.add_trace(go.Scatter(x=df_hist['date'], y=df_hist['bb_low'],
                                                  line=dict(color='red', width=1, dash='dot'), name='BB Low'))
                    # Buy signals
                    buy_hist = df_hist[df_hist["signal"] == 1]
                    sell_hist = df_hist[df_hist["signal"] == -1]
                    fig_hist.add_trace(go.Scatter(x=buy_hist['date'], y=buy_hist['close'],
                                                  mode='markers', marker=dict(symbol='triangle-up', color='green', size=12),
                                                  name='Buy Signal'))
                    fig_hist.add_trace(go.Scatter(x=sell_hist['date'], y=sell_hist['close'],
                ...
                while st.session_state["live_running"]:
                    try:
                        ...
                        recommendation = ("STRONG BUY" if score >= 4 else "BUY" if score >= 2 else "HOLD"
                                          if score > -2 else "SELL" if score > -4 else "STRONG SELL")
                        ...
                        # Live Buy/Sell markers
                        buy_live = df_plot_live[df_plot_live["signal"] == 1]
                        sell_live = df_plot_live[df_plot_live["signal"] == -1]
                        fig_live.add_trace(go.Scatter(x=buy_live['date'], y=buy_live['close'],
                                                      mode='markers', marker=dict(symbol='triangle-up', color='green', size=10),
                                                      name='Buy Signal'))
                        fig_live.add_trace(go.Scatter(x=sell_live['date'], y=sell_live['close'],
                                                      mode='markers', marker=dict(symbol='triangle-down', color='red', size=10),
                                                      name='Sell Signal'))

                        fig_live.update_layout(xaxis_title="Date", yaxis_title="Price",
