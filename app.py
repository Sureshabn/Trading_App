...
if st.session_state["access_token"]:
    ...
    # ---- Historical Analysis ----
    if st.button("Run Historical Analysis") and token:
        try:
            ...
            if df_hist.empty:
                ...
                # Buy/Sell markers
                buy_signals = df_hist[df_hist["signal"] == 1]
                sell_signals = df_hist[df_hist["signal"] == -1]
                ...
                fig_hist.update_layout(xaxis_rangeslider_visible=True, height=600)
                st.plotly_chart(fig_hist, use_container_width=True)
                st.dataframe(df_hist.tail(50).sort_values("date", ascending=False), use_container_width=True)
        except Exception as e:
            st.error(f"Error fetching historical data: {e}")

    # ---- Live Analysis ----
    if st.button("Start Live Analysis") and token:
